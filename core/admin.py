...
class AfectadoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Núcleo Familiar', {
            'fields': (
                ('cantidad_ninos', 'cantidad_adultos', 'cantidad_adultos_mayores'),
                'tiene_mascotas',
            )
        }),
        ('Documentación', {
    ...
    actions = ['aprobar_afectados', 'rechazar_afectados', 'marcar_en_revision_afectados']
    ...
    def cantidad_total_familiar(self, obj):
        return obj.cantidad_total()
    cantidad_total_familiar.short_description = 'Total Familia'
    ...
    def ver_documentos_afectado(self, obj):
        links = []
        ...
        if obj.residencia_file:
            links.append(
                f'<a href="{obj.residencia_file.url}" target="_blank" style="background:#4F46E5;color:white;padding:2px 10px;border-radius:4px;text-decoration:none;display:inline-block;margin:2px 0;">📄 Ver Comprobante</a>'
            )
        else:
            links.append('<span style="color:#DC2626;">❌ Sin comprobante</span>')
            
        return mark_safe('<br>'.join(links))
    ver_documentos_afectado.short_description = 'Documentos'
    
    def acciones_verificacion(self, obj):
        if obj.estado_verificacion not in ['aprobado', 'rechazado']:
            aprobar_url = reverse('admin:aprobar_afectado_direct', args=[obj.id])
            rechazar_url = reverse('admin:rechazar_afectado_direct', args=[obj.id])
            corregir_url = reverse('admin:solicitar_correccion_afectado_direct', args=[obj.id])
            revision_url = reverse('admin:marcar_en_revision_afectado_direct', args=[obj.id])
    ...
    def aprobar_afectados(self, request, queryset):
        count = 0
        for afectado in queryset:
            if afectado.estado_verificacion != 'aprobado':
                afectado.estado_verificacion = 'aprobado'
                afectado.fecha_verificacion = timezone.now()
                afectado.save()
                count += 1
        self.message_user(request, f'✅ {count} afectados aprobados correctamente.')
    aprobar_afectados.short_description = '✅ Aprobar afectados seleccionados'
    
    def rechazar_afectados(self, request, queryset):
        count = 0
        for afectado in queryset:
            if afectado.estado_verificacion != 'rechazado':
                afectado.estado_verificacion = 'rechazado'
                afectado.fecha_verificacion = timezone.now()
                afectado.save()
                count += 1
        self.message_user(request, f'❌ {count} afectados rechazados.')
    rechazar_afectados.short_description = '❌ Rechazar afectados seleccionados'
    
    def marcar_en_revision_afectados(self, request, queryset):
        count = 0
        for afectado in queryset:
            if afectado.estado_verificacion == 'pendiente':
                afectado.estado_verificacion = 'en_revision'
                afectado.save()
                count += 1
        self.message_user(request, f'🔍 {count} afectados marcados como "En Revisión".')
    marcar_en_revision_afectados.short_description = '🔍 Marcar como "En Revisión"'
    
    def save_model(self, request, obj, form, change):
        if change:
            original = Afectado.objects.get(pk=obj.pk)
            if original.estado_verificacion != obj.estado_verificacion:
                if obj.estado_verificacion in ['aprobado', 'rechazado']:
                    obj.fecha_verificacion = timezone.now()
                elif obj.estado_verificacion == 'pendiente':
    ...
    def get_urls(self):
        ...
        def aprobar_afectado_direct(request, pk):
            afectado = get_object_or_404(Afectado, pk=pk)
            if afectado.estado_verificacion != 'aprobado':
                afectado.estado_verificacion = 'aprobado'
                afectado.fecha_verificacion = timezone.now()
                afectado.comentario_verificacion = 'Aprobado por administrador.'
                afectado.save()
                messages.success(request, f'✅ Afectado {afectado.nombres} {afectado.apellidos} aprobado.')
            return HttpResponseRedirect(reverse('admin:core_afectado_changelist'))
        
        def rechazar_afectado_direct(request, pk):
            afectado = get_object_or_404(Afectado, pk=pk)
            if afectado.estado_verificacion != 'rechazado':
                afectado.estado_verificacion = 'rechazado'
                afectado.fecha_verificacion = timezone.now()
                afectado.comentario_verificacion = 'Rechazado por administrador.'
                afectado.save()
                messages.warning(request, f'❌ Afectado {afectado.nombres} {afectado.apellidos} rechazado.')
            return HttpResponseRedirect(reverse('admin:core_afectado_changelist'))
        
        def solicitar_correccion_afectado_direct(request, pk):
            afectado = get_object_or_404(Afectado, pk=pk)
            afectado.estado_verificacion = 'requiere_correccion'
            afectado.comentario_verificacion = 'Se requieren correcciones. Por favor contacta al administrador.'
            afectado.save()
            messages.info(request, f'📝 Se ha solicitado corrección para {afectado.nombres} {afectado.apellidos}.')
            return HttpResponseRedirect(reverse('admin:core_afectado_changelist'))
        
        def marcar_en_revision_afectado_direct(request, pk):
            afectado = get_object_or_404(Afectado, pk=pk)
            afectado.estado_verificacion = 'en_revision'
            afectado.save()
        ...
        custom_urls = [
            path('aprobar/<int:pk>/', self.admin_site.admin_view(aprobar_afectado_direct), name='aprobar_afectado_direct'),
            path('rechazar/<int:pk>/', self.admin_site.admin_view(rechazar_afectado_direct), name='rechazar_afectado_direct'),
            path('corregir/<int:pk>/', self.admin_site.admin_view(solicitar_correccion_afectado_direct), name='solicitar_correccion_afectado_direct'),
